...
from jittor.nn import ConvTranspose, conv2d, init
import functools
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                self.real_label_var = jittor.full(input.size(), self.real_label, requires_grad=False, device=self.device)
            target_tensor = self.real_label_var
        else:
            create_label = ((self.fake_label_var is None) or
                            (self.fake_label_var.numel() != input.numel()))
            if create_label:
                self.fake_label_var = jittor.full(input.size(), self.fake_label, requires_grad=False, device=self.device)
...
class UnetSkipConnectionBlock(nn.Module):
    ...
    def execute(self, x):
        if self.outermost:
            ...
            return jittor.contrib.concat([x, self.model(x)], 1)
