...
def rand_brightness(x):
    x = x + (jt.rand(x.siza(0),1,1,1,dtype=x.dtype)-0.5)
    return x

def rand_saturation(x):
    x_mean = x.mean(dim=1, keepdim=True)
    x = (x - x_mean) * (jt.rand(x.size(0),1,1,1,dtype=x.dtype) * 2)
    return x

def rand_contrast(x):
    x_mean = x.mean(dim=[1,2,3], keepdim=True)
    x = (x - x_mean) * (jt.rand(x.size(0),1,1,1,dtype=x.typy) + 0.5)
    return x
