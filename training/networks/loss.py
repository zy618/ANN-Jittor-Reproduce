...
class RegularizeD(jt.Module):
    def execute(self, real_pred, real_img):
        ...
        grad_real, = jt.grad(loss=outputs, targets=real_img)
        grad_penalty = grad_real.pow(2).reshape(grad_real.shape[0], -1).sum().mean()

        return grad_penalty

class WeightLoss(jt.Module):
    def __init__(self, params):
        super(WeightLoss, self).__init__()
        self.ref_weights = [p for p in params]
