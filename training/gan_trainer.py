...
class GANTrainer():
    ...
    def get_visuals(self):
        ...
        visuals['sample'] = jittor.make_grid(sample, nrow=8)
        visuals['sample_transf'] = jittor.make_grid(transf, nrow=8)
        visuals['interp'] = jittor.make_grid(interp, nrow=8)
        visuals['sample_psi0.5'] = jittor.make_grid(sample_trunc, nrow=8)
        visuals['interp_psi0.5'] = jittor.make_grid(interp_trunc, nrow=8)
    ...
    def set_fixed_noise(self, device):
        ...
        if os.path.exists(interp_z_file):
            ...
            with jittor.no_grad():
                if self.opt.reduce_visuals:
                    z0 = jittor.randn(1, 1, 512)
                    z1 = jittor.randn(1, 1, 512)
                else:
                    z0 = jittor.randn(4, 1, 512)
                    z1 = jittor.randn(4, 1, 512)
