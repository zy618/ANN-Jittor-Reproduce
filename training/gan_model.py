...
class GANModel(jittor.Module):
    def __init__(self, opt):
        super().__init__()
        self.opt = opt
        self.FloatTensor = jittor.float32
        self.ByteTensor = jittor
    ...
    def create_optimizers(self, opt):
        ...
        optimizer_G = jittor.optim.Adam(G_params, lr=G_lr, betas=(G_beta1, G_beta2))
        optimizer_D = jittor.optim.Adam(D_params, lr=D_lr, betas=(D_beta1, D_beta2))
    ...
    def save(self, iters):
        ...
        jittor.save(self.netG.state_dict(), save_path + "G.pth")
        jittor.save(self.netD_sketch.state_dict(), save_path + "D_sketch.pth")
        if self.opt.l_image > 0:
            jittor.save(self.netD_image.state_dict(), save_path + "D_image.pth")

    def load(self, iters):
        load_path = os.path.join(self.opt.checkpoints_dir, self.opt.name, f"{iters}_net_")
        state_dict_g = jittor.load(load_path + "G.pth", map_location=self.device)
        self.netG.load_state_dict(state_dict_g)

        state_dict_d_sketch = jittor.load(load_path + "D_sketch.pth", map_location=self.device)
        self.netD_sketch.load_state_dict(state_dict_d_sketch)
        if self.opt.l_image > 0:
            state_dict_d_image = jittor.load(load_path + "D_image.pth", map_location=self.device)
    ...
    def initialize_networks(self, opt):
        ...
        if opt.g_pretrained != '':
            weights = jittor.load(opt.g_pretrained, map_location=lambda storage, loc: storage)
        ...
        if netD_sketch is not None and opt.d_pretrained != '' and not opt.dsketch_no_pretrain:
            ...
            weights = jittor.load(opt.d_pretrained, map_location=lambda storage, loc: storage)
...
def mixing_noise(batch, latent_dim, prob, device):
    ...
    if prob > 0 and random.random() < prob:
        return jittor.randn(2, batch, latent_dim, device=device).unbind(0)
    else:
        return [jittor.randn(batch, latent_dim, device=device)]
