...
def save_image_jittor(img, name):
    """Helper function to save jittor tensor into an image file."""
    jittor.save_image(
        img,
        name,
        nrow=1,
...
def generate(args, netG, device, mean_latent):
    ...
    if args.w_shift is not None:
        w_shift = jittor.from_numpy(np.load(args.w_shift))
        w_shift = w_shift[None, :]
        mean_latent = mean_latent + w_shift
    else:
        w_shift = jittor.float32(0.)
    ...
    with jittor.no_grad():
        ...
        if args.fixed_z is not None:
            sample_z = jittor.load(args.fixed_z, map_location=device) + w_shift
            for start in range(0, sample_z.size(0), args.batch_size):
                ...
                for s in sample:
                    save_image_jittor(s, f'{args.save_dir}/{str(ind).zfill(6)}.png')
                    ind += 1
            return
        ...
        for start in range(0, args.samples, args.batch_size):
            ...
            batch_sz = end - start
            sample_z = jittor.randn(batch_sz, 512, device=device) + w_shift
...
if __name__ == '__main__':
    ...
    checkpoint = jittor.load(args.ckpt, map_location='cpu')
