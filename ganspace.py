...
def gen_principal_components(dump_name, device='cpu'):
    ...
    with np.load(dump_name) as data:
        lat_comp = jittor.numpy2cupy(data['lat_comp'])
        lat_mean = jittor.numpy2cupy(data['lat_mean'])
...
if __name__ == '__main__':
    ...
    checkpoint = jittor.load(args.ckpt)
    ...
    if args.fixed_z is None:
        z = jittor.randn(args.samples, 512)
    else:
        z = jittor.load(args.fixed_z, map_location='cpu')
